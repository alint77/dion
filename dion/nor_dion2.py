...
class NorDion2(torch.optim.Optimizer):
    """
    NorDion2 optimizer: Dion2 submatrix selection + error feedback, combined with
    NorMuon variance reduction. DDP-only implementation.
        ...
        if params_list and isinstance(params_list[0], dict):
            ...
            for group in params_list:
                ...
                for params in (groups_by_label.values() if has_label else [group_params]):
                    ...
                    if "muon_beta2" in group and "beta2" not in group:
                        group["beta2"] = group["muon_beta2"]
                    ...
                    group.setdefault("beta2", defaults["beta2"])
    ...
    def step_p2(self):
        ...
        for group, info in zip(self.param_groups, group_infos):
            ...
            if num_params == 0:
                ...
                if "second_momentum_buffer" not in group or group["second_momentum_buffer"].shape != (
                    v_selected[..., :, :1].shape if red_dim == -1 else v_selected[..., :1, :].shape
                ):
                    group["second_momentum_buffer"] = (
                        torch.zeros_like(v_selected[..., :, :1], dtype=torch.float32)
                        if red_dim == -1
                        else torch.zeros_like(v_selected[..., :1, :], dtype=torch.float32)
                    )
                second_momentum_buffer = group["second_momentum_buffer"]
                v_selected = apply_normuon_variance_reduction(
                    v_selected, second_momentum_buffer, group["beta2"], red_dim
                )
