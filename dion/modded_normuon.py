...
# -----------------------------------------------------------------------------
# Compiled helpers for Modded NorMuon
...
class ModdedNorMuon(torch.optim.Optimizer):
    """
    Modded NorMuon optimizer adapted from modded-nanogpt. DDP-only implementation.
            ...
            for group in params_list:
                ...
                for params in (groups_by_label.values() if has_label else [group_params]):
                    ...
                    if "chunk_size" not in group:
                        group["chunk_size"] = (len(group["params"]) + self.world_size - 1) // self.world_size
    ...
    def step_p2(self):
        ...
        for group, info in zip(self.param_groups, group_infos):
            ...
            # is 'incorrect' for O. However, correcting this showed no improvement.
